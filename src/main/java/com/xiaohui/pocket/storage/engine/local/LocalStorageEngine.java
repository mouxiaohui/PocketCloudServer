...
public class LocalStorageEngine extends AbstractStorageEngine {
    ...
    public String storeChunk(StoreFileChunkDto storeFileChunkDto) throws IOException {
        ...
        FileUtils.writeStreamToFile(storeFileChunkDto.getInputStream(), new File(realFilePath), storeFileChunkDto.getTotalSize());
        return realFilePath;
    }
    ...
    public String mergeFile(MergeFileDto mergeFileDto) throws IOException {
        checkMergeFileContext(mergeFileDto);
     ...
     */
    @Override
    public void readFile(ReadFileDto readFileDto) throws IOException {
        checkReadFileContext(readFileDto);
        File file = new File(readFileDto.getRealPath());
        FileUtils.writeFileToOutputStream(new FileInputStream(file), readFileDto.getOutputStream(), file.length());
    }
